...
fn post_csv(url: &str, form: &HashMap<&str, String>) -> Result<String> {
    let client = Client::builder().gzip(true).brotli(true).deflate(true).build()?;
...
pub fn fetch_knmi_hourly_to_df(start: &str, end: &str, stns: &str, vars: &str) -> Result<DataFrame> {
...
fn normalize_date_str(df: &DataFrame) -> Result<(DataFrame, String)> {
    ...
    let date_col = candidate.ok_or_else(|| anyhow!(
        "KNMI date column not found; columns: {:?}", df.get_column_names()
    ))?;
...
pub fn write_partitioned_by_date<P: AsRef<Path>>(df: &DataFrame, date_col: &str, out_dir: P) -> Result<()> {
    let out_dir = out_dir.as_ref();
    std::fs::create_dir_all(out_dir)?;

    // Normalize/ensure a YYYY-MM-DD string column exists
    let (df2, date_col_norm) = if date_col == "date_str" && df.get_column_names().iter().any(|n| n == &"date_str") {
